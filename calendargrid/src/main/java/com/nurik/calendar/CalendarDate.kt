...
class CalendarDate(date: Date) : Parcelable, Comparable<CalendarDate> {
    ...
    val week: Int get() = _calendar.get(Calendar.WEEK_OF_YEAR)
    ...
    fun weeksBetween(other: CalendarDate): Int {
        val d1i = Instant.ofEpochMilli(this.timeInMillis)
        val d2i = Instant.ofEpochMilli(other.timeInMillis)

        val startDate = LocalDateTime.ofInstant(d1i, ZoneId.systemDefault())
        val endDate = LocalDateTime.ofInstant(d2i, ZoneId.systemDefault())

        return ChronoUnit.WEEKS.between(startDate, endDate).toInt()
    }
