...
import com.nurik.calendar.CalendarView.SelectionMode.NONE
import com.nurik.calendar.adapter.CalendarAdapter
import com.nurik.calendar.adapter.CalendarItemsGenerator
import com.nurik.calendar.adapter.CalendarItemsGenerator_OLD
...
import com.nurik.calendar.adapter.manager.CalendarAdapterDataManager
import com.nurik.calendar.decorations.GridDividerItemDecoration
import com.nurik.calendar.extension.getColorInt
import com.nurik.calendar.selection.*
import com.nurik.calendar.style.CalendarStyleAttributes
import com.nurik.calendar.style.CalendarStyleAttributesReader
import com.nurik.calendar.utils.DateInfoProvider
import com.nurik.calendar.utils.DisplayedDatesRangeFactory
import getColorInt
...
class CalendarView @JvmOverloads constructor(
    ...
    companion object {
        ...
        private const val BUNDLE_SUPER_STATE = "com.nurik.calendar.super_state"
        private const val BUNDLE_DISPLAYED_DATE = "com.nurik.calendar.displayed_date"
        private const val BUNDLE_DISPLAY_DATE_RANGE = "com.nurik.calendar.display_date_range"
        private const val BUNDLE_LIMIT_DATE_RANGE = "com.nurik.calendar.limit_date_range"
        private const val BUNDLE_SELECTION_MODE = "com.nurik.calendar.selection_mode"
        private const val BUNDLE_FIRST_DAY_OF_WEEK = "com.nurik.calendar.first_day_of_week"
        private const val BUNDLE_SHOW_YEAR_SELECTION_VIEW =
            "com.nurik.calendar.show_year_selection_view"
    }
    ...
    private val calendarHeader: TextView
    ...
    private lateinit var calendarItemsGenerator: CalendarItemsGenerator_OLD
    ...
    private var firstDayOfWeek: Int? = null
        set(value) {
            ...
            calendarItemsGenerator = CalendarItemsGenerator_OLD(firstDayOfWeek ?: defaultFirstDayOfWeek)
        }
    ...
    init {
        ...
        calendarHeader = findViewById(R.id.calendar_year_text)
        ...
        //TODO acertar para ser grid continuo ou grid separado por mes
    ...
    fun getMyAdapter(): CalendarAdapter{
    ...
    fun setupCalendar(
            ...
            list : MutableList<CalendarDayModel>? = mutableListOf()
    ) {
        ...
        if(list!=null){
            this.datesIndicators = generateCalendarDateIndicators(list!!)
        }
    ...
    fun moveToDate(date: CalendarDate) {
        ...
        if (dateMonthPosition != -1) {
            ...
            gridLayoutManager.scrollToPositionWithOffset(dateMonthPosition, 10)
    ...
    var onScrollListener: () -> Unit = {}


   /* private fun setOnScrollListener() {
        if(mycalendar.getRecycler()!=null && mycalendar.getRecycler().adapter!=null){
            mycalendar.getRecycler().addOnScrollListener(object : RecyclerView.OnScrollListener() {
...
*/
    ...
    private inner class CalendarItemsGenerationListener : RecyclerView.OnScrollListener() {
        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
            ...
            setMonthYearTitle(recyclerView, gridLayoutManager.findFirstVisibleItemPosition())
            onScrollListener()
        }
    }

    private fun setMonthYearTitle(recyclerView: RecyclerView, i: Int) {
        var day = (recyclerView.adapter as CalendarAdapter).getItemAtPosition(i + 7)
        if(day !is EmptyItem){
            if(day is DateItem){
                var item = day
                var months: Array<String> = resources.getStringArray(R.array.months)
                calendarHeader.setText(months[item.date.month] + " " + item.date.year)
            }else{
                //calendarHeader.visibility = View.GONE
            }
        }
    ...
    private fun generateCalendarDateIndicators(list : MutableList<CalendarDayModel>): List<CalendarView.DateIndicator> {
        val indicators = mutableListOf<CalendarView.DateIndicator>()

        for(item in list) {
            if(item.child!=null){
                for (child in item.child!!) {
                    var names = resources.getStringArray(R.array.channels_name)
                    indicators += CalendarDateIndicator(
                            eventName = names[child.media - 1],
                            date = CalendarDate(SimpleDateFormat("yyyy-MM-dd").parse(child.date)),
                            color = getColorInt(child.mediaColorId!!)
                    )
                }
            }
        }
